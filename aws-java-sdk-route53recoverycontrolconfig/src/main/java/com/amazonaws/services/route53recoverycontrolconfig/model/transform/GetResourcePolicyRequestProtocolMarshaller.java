...
public class GetResourcePolicyRequestProtocolMarshaller implements Marshaller<Request<GetResourcePolicyRequest>, GetResourcePolicyRequest> {
    ...
    private static final OperationInfo SDK_OPERATION_BINDING = OperationInfo.builder().protocol(Protocol.REST_JSON).requestUri("/resourcePolicy/{ResourceArn}")
            .httpMethodName(HttpMethodName.GET).hasExplicitPayloadMember(false).hasPayloadMembers(false).serviceName("AWSRoute53RecoveryControlConfig").build();
