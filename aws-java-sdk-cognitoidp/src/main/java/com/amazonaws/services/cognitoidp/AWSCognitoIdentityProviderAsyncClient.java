 ...
 * <p>
 * <p>
 * With the Amazon Cognito user pools API, you can set up user pools and app clients, and authenticate users. To
 * authenticate users from third-party identity providers (IdPs) in this API, you can <a href=
 ...
 * </p>
 * <p>
 * This API reference provides detailed information about API operations and object types in Amazon Cognito. At the
 * bottom of the page for each API operation and object, under <i>See Also</i>, you can learn how to use it in an Amazon
 * Web Services SDK in the language of your choice.
 * </p>
 * <p>
 * Along with resource management operations, the Amazon Cognito user pools API includes classes of operations and
 * authorization models for client-side and server-side user operations. For more information, see <a
 * href="https://docs.aws.amazon.com/cognito/latest/developerguide/user-pools-API-operations.html">Using the Amazon
 * Cognito native and OIDC APIs</a> in the <i>Amazon Cognito Developer Guide</i>.
 * </p>
 * <p>
 * You can also start reading about the <code>CognitoIdentityProvider</code> client in the following SDK guides.
 * </p>
 * <ul>
 * <li>
 * <p>
...
public class AWSCognitoIdentityProviderAsyncClient extends AWSCognitoIdentityProviderClient implements AWSCognitoIdentityProviderAsync {
     ...
     */
    AWSCognitoIdentityProviderAsyncClient(AwsAsyncClientParams asyncClientParams, boolean endpointDiscoveryEnabled) {
    ...
    @Override
    public java.util.concurrent.Future<GetLogDeliveryConfigurationResult> getLogDeliveryConfigurationAsync(final GetLogDeliveryConfigurationRequest request,
            final com.amazonaws.handlers.AsyncHandler<GetLogDeliveryConfigurationRequest, GetLogDeliveryConfigurationResult> asyncHandler) {
        final GetLogDeliveryConfigurationRequest finalRequest = beforeClientExecution(request);

        return executorService.submit(new java.util.concurrent.Callable<GetLogDeliveryConfigurationResult>() {
            @Override
            public GetLogDeliveryConfigurationResult call() throws Exception {
                GetLogDeliveryConfigurationResult result = null;

                try {
                    result = executeGetLogDeliveryConfiguration(finalRequest);
                } catch (Exception ex) {
                    if (asyncHandler != null) {
                        asyncHandler.onError(ex);
                    }
    ...
    @Override
    public java.util.concurrent.Future<SetLogDeliveryConfigurationResult> setLogDeliveryConfigurationAsync(final SetLogDeliveryConfigurationRequest request,
            final com.amazonaws.handlers.AsyncHandler<SetLogDeliveryConfigurationRequest, SetLogDeliveryConfigurationResult> asyncHandler) {
        final SetLogDeliveryConfigurationRequest finalRequest = beforeClientExecution(request);

        return executorService.submit(new java.util.concurrent.Callable<SetLogDeliveryConfigurationResult>() {
            @Override
            public SetLogDeliveryConfigurationResult call() throws Exception {
                SetLogDeliveryConfigurationResult result = null;

                try {
                    result = executeSetLogDeliveryConfiguration(finalRequest);
                } catch (Exception ex) {
                    if (asyncHandler != null) {
                        asyncHandler.onError(ex);
                    }
