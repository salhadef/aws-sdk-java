        ...
        while (true) {
            ...
            if (token == FIELD_NAME || token == START_OBJECT) {
                if (context.testExpression("s3Bucket", targetDepth)) {
                    context.nextToken();
                    destination.setS3Bucket(S3BucketJsonUnmarshaller.getInstance().unmarshall(context));
                }
                if (context.testExpression("firehoseStream", targetDepth)) {
                    context.nextToken();
                    destination.setFirehoseStream(FirehoseStreamJsonUnmarshaller.getInstance().unmarshall(context));
                }
